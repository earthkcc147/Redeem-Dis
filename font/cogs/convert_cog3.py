...
class FontMenuView(discord.ui.View):
    def __init__(self, fonts, ctx):
        ...
        self.add_item(discord.ui.Button(label="ก่อนหน้า", style=discord.ButtonStyle.secondary, callback=self.prev_page))
        self.add_item(discord.ui.Button(label="ถัดไป", style=discord.ButtonStyle.secondary, callback=self.next_page))
    ...
    async def next_page(self, interaction: discord.Interaction):
        self.current_page += 1
        self.current_page = min(self.total_pages - 1, self.current_page)
        await self.send_current_page()
...
class ConvertCog(commands.Cog):
    ...
    async def convert(self, ctx):
        ...
        async def font_list_callback(interaction: discord.Interaction):
            ...
            font_view = FontMenuView(fonts, ctx)
            await font_view.send_current_page()
        font_list_button.callback = font_list_callback
        view.add_item(font_list_button)

        await ctx.send(embed=embed, view=view)

# Modal สำหรับการกรอกข้อความ
